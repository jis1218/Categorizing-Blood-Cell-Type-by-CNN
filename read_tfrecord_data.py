...
from package.dir_traversal_tfrecord import tfrecord_auto_traversal
...
def read_and_decode(filename_queue):
    ...
    current_image_object.image = tf.image.resize_image_with_crop_or_pad(image_raw, FLAGS.image_height, FLAGS.image_width)
    ...
    current_image_object.label = tf.cast(features["image/class/label"], tf.int32)
    
    return current_image_object

filename_queue = tf.train.string_input_producer(tfrecord_auto_traversal(), shuffle = True)
...
with tf.Session() as sess:
    ...
    for i in range(FLAGS.image_number):
